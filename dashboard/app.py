...
- Inventory Analysis
- Reports & Analytics (Confidence, Training, Records, Planograms)
"""
...
with st.sidebar:
    ...
    st.markdown("<hr style='border-color: rgba(0,0,0,0.1); margin: 2rem 0;'>", unsafe_allow_html=True)
    ...
    API_BASE_URL = st.text_input(
        "API Endpoint", value=API_DEFAULT_BASE_URL, help="FastAPI backend URL", label_visibility="collapsed"
    )
    ...
    # Professional Navigation (5 main pages)
    ...
    try:
        summary_side = requests.get(f"{API_BASE_URL}/api/summary", timeout=2).json()
        st.markdown("### 📊 Quick Stats")
        st.markdown(
            f"""
        <div style='color: #212529; font-size: 0.9rem;'>
            <div style='margin: 0.5rem 0;'>
                <strong>Detections:</strong> {summary_side.get('total_detections', 0):,}
            </div>
            <div style='margin: 0.5rem 0;'>
                <strong>SKUs:</strong> {summary_side.get('unique_skus', 0)}
            </div>
            <div style='margin: 0.5rem 0;'>
                <strong>Confidence:</strong> {summary_side.get('average_confidence', 0):.1%}
            </div>
        </div>
        """,
            unsafe_allow_html=True,
...
def api_request(endpoint: str, method: str = "GET", **kwargs):
    ...
    try:
        ...
        st.error("⚠️ API Error: Request timeout. Backend may be slow or unavailable.")
        return None
    except requests.exceptions.ConnectionError:
        st.error("⚠️ API Error: Cannot connect to backend. Check if backend is running.")
        return None
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 500:
            st.error("⚠️ API Error: 500 Server Error. Backend encountered an internal error.")
        elif e.response.status_code == 404:
            st.error("⚠️ API Error: 404 Not Found. Endpoint may not exist.")
        else:
            st.error(f"⚠️ API Error: {e.response.status_code} {e.response.reason}")
        return None
...
def cached_api_request(endpoint: str, method: str = "GET", **kwargs):
    """Cached version of API request for GET endpoints (light caching)."""
    return api_request(endpoint, method=method, **kwargs)


def create_gradient_card(title: str, value: str, gradient: str, icon: str = ""):
    """Create professional gradient metric card (not heavily used now but kept)."""
    return f"""
...
with st.spinner("Loading key metrics..."):
    summary_global = cached_api_request("/api/summary")

if summary_global:
    st.markdown("### Key KPIs")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("Total Detections", f"{summary_global.get('total_detections', 0):,}")
    with c2:
        st.metric("Unique SKUs", f"{summary_global.get('unique_skus', 0)}")
    with c3:
        st.metric("Avg Confidence", f"{summary_global.get('average_confidence', 0):.1%}")
    with c4:
        # Static from your study; you can update if you pull this from API
        st.metric("Fine-Tuned mAP50", "4.04%")
...
if "Overview" in page:
    ...
    <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
        ...
        <div style='text-align: center; color: white;'>
            <h1 style='color: white !important; font-size: 2.5rem; font-weight: 700; margin-bottom: 1rem;'>
                📷 VisionStock
            </h1>
            <p style='color: rgba(255, 255, 255, 0.95) !important; font-size: 1.2rem; margin-bottom: 1rem; font-weight: 500;'>
                AI-Powered Retail Inventory Detection System
            </p>
            <p style='color: rgba(255, 255, 255, 0.9) !important; font-size: 1rem; max-width: 800px; margin: 0 auto; line-height: 1.6;'>
                Transform your retail operations with computer vision. Automatically detect products on shelves, 
                track inventory levels, and prevent stockouts with our fine-tuned YOLOv8 detection system.
            </p>
        </div>
    </div>
    """,
    ...
    st.markdown("### 🎯 About VisionStock")
    st.markdown(
        """
    **VisionStock** is an end-to-end computer vision system that addresses the critical problem of automated retail shelf inventory management. 
    Using fine-tuned YOLOv8 object detection, the system automatically identifies products on retail shelves, compares detections against 
    planogram expectations, and provides real-time inventory analytics.
    
    **Research Question**: *"Does fine-tuning YOLOv8 on a small, category-specific retail dataset significantly improve product detection 
    performance on shelf images compared to baseline pre-trained models?"*
    """
    )
    ...
    with col1:
        st.markdown(
            """
        **🔬 Two-Study Evaluation**
        - Comprehensive baseline vs fine-tuned comparison
        - Study 1: Different datasets (SKU-110K baseline, Custom fine-tuned)
        - Study 2: Same dataset (Custom baseline, Custom fine-tuned)
        
        **🤖 AI-Powered Detection**
        - Fine-tuned YOLOv8n model (34 product classes)
        - Real-time object detection on shelf images
        - Baseline and fine-tuned model comparison
        
        **📊 Real-Time Analytics**
        - Live inventory status tracking
        - Stock discrepancy detection (missing, low stock, overstock)
    ...
    with col2:
        ...
        **📦 Inventory Management**
        - Planogram-based inventory comparison
        - Expected vs detected product counts
        ...
        **📈 Comprehensive Dashboard**
        ...
        - Training summary and analytics
    ...
    with tech_col4:
        ...
        st.markdown("- Docker")
        st.markdown("- GCP Cloud Run")
        st.markdown("- Ultralytics Hub")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.info("💡 See **Model Performance** page for detailed comparison and metrics.")
    st.markdown("</div>", unsafe_allow_html=True)

    # Real-Time Statistics
    summary = summary_global or cached_api_request("/api/summary")
    if summary:
        st.markdown("<div class='section-card'>", unsafe_allow_html=True)
        ...
        detections = cached_api_request("/api/detections", params={"limit": 50})
        ...
        if detections:
            ...
            if detection_times:
                df_timeline = pd.DataFrame({"timestamp": detection_times, "count": [1] * len(detection_times)})
                df_timeline = df_timeline.set_index("timestamp").resample("1h").sum().reset_index()

                fig = px.area(df_timeline, x="timestamp", y="count", title="", color_discrete_sequence=["#667eea"])
                fig.update_layout(
                ...
                last_ts = detections[0].get("timestamp")
                if last_ts:
                    st.caption(f"Last detection record: `{last_ts}`")
        else:
            st.info("No detection history yet. Run detections to populate this chart.")
        st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.info("👆 Connect to the API to view real-time statistics")
    ...
    - ✅ Real-time inventory tracking with automated discrepancy detection
    """
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
# 2. MODEL PERFORMANCE (Before vs After) - MOST IMPORTANT
            ...
            if study1_path.exists():
                ...
                fig1.add_trace(
                    go.Bar(
                        ...
                        y=[
                            finetuned["mAP50"] * 100,
                            finetuned["mAP50_95"] * 100,
                            finetuned["precision"] * 100,
                            finetuned["recall"] * 100,
                            finetuned["f1_score"] * 100,
                        ],
                        marker_color="#764ba2",
                    )
                )
                ...
                **Study 1 Insights:**
                ...
                st.warning("⚠️ Study 1 results not found. Place `results/study1_comparison.json` in the project.")
            ...
            if study2_path.exists():
                ...
                fig2.add_trace(
                    go.Bar(
                        ...
                        y=[
                            baseline2["mAP50"] * 100,
                            baseline2["mAP50_95"] * 100,
                            baseline2["precision"] * 100,
                            baseline2["recall"] * 100,
                            baseline2["f1_score"] * 100,
                        ],
                        marker_color="#e74c3c",
                    )
                )
                fig2.add_trace(
                    go.Bar(
                        ...
                        x=comparison_df2["Metric"],
                        y=[
                            finetuned2["mAP50"] * 100,
                            finetuned2["mAP50_95"] * 100,
                            finetuned2["precision"] * 100,
                            finetuned2["recall"] * 100,
                            finetuned2["f1_score"] * 100,
                        ],
                        marker_color="#27ae60",
                    )
                )
                ...
                st.warning("⚠️ Study 2 results not found. Place `results/study2_comparison.json` in the project.")
        except Exception as e:
            st.error(f"Error loading Study 2: {e}")

    # Model comparison via API (auto part)
    with st.spinner("Loading model comparison data..."):
        models = cached_api_request("/api/models")
    ...
    if models:
        ...
        if baseline_models and finetuned_models:
            st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
            st.markdown("### 🔁 API Model Comparison (Baseline vs Fine-Tuned)")
            ...
            with col2:
                ...
                selected_finetuned = st.selectbox(
                    "🎯 **Fine-Tuned Model**", list(finetuned_options.keys()), help="Select the fine-tuned model"
                )
                finetuned_id = finetuned_options[selected_finetuned]

            auto_compare = st.checkbox(
                "🔄 Auto-load comparison", value=True, help="Automatically show comparison when models are selected"
            )

            run_compare = auto_compare or st.button("🔄 **Compare Models**", use_container_width=True, type="primary")

            if run_compare:
                comparison = api_request(
                    "/api/models/comparison", params={"baseline_id": baseline_id, "finetuned_id": finetuned_id}
                )
                ...
                if comparison:
                    ...
                    improvement_prec = (
                        (
                            comparison["improvements"]["precision_improvement"]
                            / comparison["baseline"]["precision"]
                            * 100
                        )
                        if comparison["baseline"]["precision"] > 0
                        else 0
                    )
                    ...
                    improvement_recall = (
                        (comparison["improvements"]["recall_improvement"] / comparison["baseline"]["recall"] * 100)
                        if comparison["baseline"]["recall"] > 0
                        else 0
                    )
                    ...
                    improvement_f1 = (
                        (comparison["improvements"]["f1_improvement"] / comparison["baseline"]["f1_score"] * 100)
                        if comparison["baseline"]["f1_score"] > 0
                        else 0
                    )

                    with col1:
                        st.metric(
                            "mAP50", f"{finetuned_map50:.1f}%", f"+{improvement_map50:.1f}%", delta_color="normal"
                        )

                    with col2:
                        st.metric(
                            "mAP50-95",
                            f"{finetuned_map50_95:.1f}%",
                    ...
                    with col3:
                        st.metric(
                            "Precision", f"{finetuned_prec:.1f}%", f"+{improvement_prec:.1f}%", delta_color="normal"
                        )

                    with col4:
                        st.metric(
                            "Recall", f"{finetuned_recall:.1f}%", f"+{improvement_recall:.1f}%", delta_color="normal"
                        )
                    ...
                    success_df = pd.DataFrame(
                        [
                            ...
                            },
                            {
                                ...
                                "Target": "≤ 2000ms",
                                "Achieved": (
                                    f"{comparison['finetuned']['inference_time_ms']:.2f}ms"
                                    if comparison["finetuned"]["inference_time_ms"]
                                    else "N/A"
                                ),
                                "Status": (
                                    "✅ PASS"
                                    if comparison["finetuned"]["inference_time_ms"]
                                    and comparison["finetuned"]["inference_time_ms"] <= 2000
                                    else "❌ FAIL"
                                ),
                            },
                        ]
            ...
            st.warning("⚠️ Please register both baseline and fine-tuned models via API.")
    else:
        st.info("ℹ️ No models registered yet. Run training and model registration first.")
...
# 3. YOLO DETECTION VISUALIZER
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            st.image(uploaded_file, caption="Baseline Detection")

            if st.button("🔍 Run Baseline Detection", use_container_width=True, type="primary", key="baseline_btn"):
                with st.spinner("Running baseline detection..."):
                    ...
                    result = api_request(
                        ...
                        method="POST",
                        files=files,
                        params={"shelf_location": "A1", "model_type": "baseline"},
                    )
                    if result:
                        st.success(f"✅ Detected {result.get('detections_count', 0)} objects (baseline)")
                        ...
                        if baseline_detections:
                            ...
                            st.info("ℹ️ No detections returned")
                    else:
                        st.warning("⚠️ Detection failed. Check backend logs.")

        with col2:
            st.markdown("### 🎯 Fine-Tuned Prediction")
            st.markdown("*Fine-tuned on custom dataset*")
            st.image(uploaded_file, caption="Fine-Tuned Detection")

            if st.button("🔍 Run Fine-Tuned Detection", use_container_width=True, type="primary", key="finetuned_btn"):
                with st.spinner("Running fine-tuned detection..."):
                    uploaded_file.seek(0)  # Reset file pointer - FIXED!
                    files = {"file": (uploaded_file.name, uploaded_file, uploaded_file.type)}
                    result_ft = api_request(
                        "/api/detect",
                        method="POST",
                        files=files,
                        params={"shelf_location": "A1", "model_type": "finetuned"},
                    )
                    if result_ft:
                        st.success(f"✅ Detected {result_ft.get('detections_count', 0)} objects (fine-tuned)")
                        ft_detections = result_ft.get("detections", [])
                        if ft_detections:
                            st.dataframe(
                                pd.DataFrame(ft_detections)[["class_name", "confidence"]],
                                use_container_width=True,
                                hide_index=True,
                            )
                        else:
...
# 4. INVENTORY ANALYSIS
    ...
    planogram_name = st.selectbox(
        "📋 **Select Planogram**", ["Shelf_A1", "Shelf_A2", "Shelf_B1"], help="Select a planogram to analyze"
    )

    if st.button("🔍 **Analyze Shelf Inventory**", use_container_width=True, type="primary"):
        with st.spinner("Analyzing shelf inventory..."):
            planograms = cached_api_request("/api/planograms", params={"planogram_name": planogram_name})
            detections = cached_api_request("/api/detections", params={"limit": 1000})

            inventory_data = []
            if not planograms or not detections:
                # Seed / fallback data
                st.warning("⚠️ Live data not available — showing seeded demo inventory.")
                inventory_data = [
                    {"SKU": "COKE_330", "Product": "Coke Can 330ml", "Expected": 10, "Detected": 8},
                    {"SKU": "LAYS_SM", "Product": "Lays Salted", "Expected": 12, "Detected": 12},
                    {"SKU": "DETERGENT_X", "Product": "Detergent X 1L", "Expected": 6, "Detected": 3},
                    {"SKU": "SHAMPOO_Y", "Product": "Shampoo Y 250ml", "Expected": 8, "Detected": 0},
                ]
                for row in inventory_data:
                    expected = row["Expected"]
                    detected_count = row["Detected"]
                    if detected_count == 0:
                        status = "❌ Missing"
                        ...
                        status = "✅ OK"
                    row["Difference"] = detected_count - expected
                    row["Status"] = status
            else:
            ...
            st.download_button(
                "⬇️ Download Inventory Report (CSV)",
                csv,
                f"visionstock_inventory_{planogram_name}.csv",
                "text/csv",
                use_container_width=True,
            )
            st.markdown("</div>", unsafe_allow_html=True)
...
# 5. REPORTS & ANALYTICS (Tabs)
elif "Reports & Analytics" in page:
    st.markdown("## 📈 Reports & Analytics")
    st.markdown("**Deep dive into confidence, training, detections, and planograms.**")
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
        ...
        if detections:
            ...
            if confidences:
                ...
                with col1:
                    ...
                    high_conf = sum(1 for c in confidences if c > threshold)
                    total = len(confidences)
                    pct_high = (high_conf / total * 100) if total > 0 else 0
                    st.metric(f"High Confidence (>{threshold:.2f})", f"{high_conf:,}/{total:,}", f"{pct_high:.1f}%")
                ...
                st.info(f"💡 **{pct_high:.1f}%** of detections have confidence above your chosen threshold.")
            else:
    ...
    with tab2:
        ...
        with st.spinner("Loading training information..."):
            models = cached_api_request("/api/models", params={"model_type": "finetuned"})
        ...
        if models and len(models) > 0:
            ...
            **See "Model Performance" page for detailed comparison of both studies!**
            ...
            st.info(
                "💡 Training curves (loss vs epochs) are generated by Ultralytics at `runs/detect/[experiment_name]/results.png`."
            )

            training_curve = st.file_uploader("📊 Upload Training Curve (results.png)", type=["png", "jpg"])
            if training_curve:
                st.image(training_curve, caption="Training Loss Curve")
            ...
            st.info("💡 Run the training script and register models to populate this section.")

    # -----------------------------------------------------------------------
    # TAB 3: DETECTION RECORDS
    # -----------------------------------------------------------------------
    with tab3:
        st.markdown("### 🔍 Detection Records")
        st.markdown("**View detection records from the fine-tuned model**")
        ...
        with st.expander("ℹ️ **Model Information**", expanded=False):
            ...
            **Using Fine-Tuned Model (Study 2 Results):**
            ...
            - Detections below are produced by the fine-tuned model
            """
            )
        ...
        with st.spinner("Loading detections..."):
            detections = cached_api_request("/api/detections", params=params)
        ...
        if detections:
                ...
                detection_data.append(
                    {
                        ...
                        "Shelf": (
                            det.get("detections", [{}])[0].get("shelf_location", "N/A")
            ...
            csv = df.to_csv(index=False).encode("utf-8")
            st.download_button(
                "⬇️ Download Detection Records (CSV)",
                csv,
                "visionstock_detections.csv",
                "text/csv",
                use_container_width=True,
            )
            ...
            st.info("ℹ️ No detection records found. Run detections to populate this table.")

    # -----------------------------------------------------------------------
    # TAB 4: PLANOGRAM MANAGEMENT
    # -----------------------------------------------------------------------
    with tab4:
        st.markdown("### 📋 Planogram Management")
        st.markdown("**Manage expected inventory and compare with detections**")
        ...
        with st.expander("ℹ️ **Model Information**", expanded=False):
            ...
            **Using Fine-Tuned Model (Study 2 Results):**
            ...
            - Planogram comparisons use detections from the fine-tuned model
            """
            )
        ...
        with st.expander("➕ Create New Planogram", expanded=False):
            with st.form("planogram_form"):
                ...
                if submitted:
                    if planogram_name and sku and shelf_location:
                        data = {
        ...
        with st.spinner("Loading planograms..."):
            planograms = cached_api_request("/api/planograms", params=params)
        ...
        if planograms:
            ...
            csv = df.to_csv(index=False).encode("utf-8")
            st.download_button(
                "⬇️ Download Planograms (CSV)",
                csv,
                "visionstock_planograms.csv",
                "text/csv",
                use_container_width=True,
            )
        else:
            st.info("ℹ️ No planograms found. Create one using the form above.")
